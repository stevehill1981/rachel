defmodule Rachel.Games.Game do
  ...
  def play_card(%__MODULE__{status: :playing} = game, player_id, card_indices) when is_list(card_indices) do
    with {:ok, player_index} <- find_player_index(game, player_id),
      ...
      new_game = if new_game.nominated_suit == :pending do
        new_game
      else
  ...
  def nominate_suit(%__MODULE__{nominated_suit: :pending} = game, player_id, suit) when suit in [:hearts, :diamonds, :clubs, :spades] do
    with {:ok, player_index} <- find_player_index(game, player_id),
  ...
  defp validate_play(%__MODULE__{pending_pickup_type: :twos} = game, cards) do
  ...
  defp validate_play(%__MODULE__{pending_pickup_type: :black_jacks} = game, cards) do
  ...
  defp validate_play(%__MODULE__{nominated_suit: suit} = game, cards) when not is_nil(suit) do
    [first_card | _] = cards
  ...
  defp apply_single_card_effect(game, card) do
    case Card.special_effect(card) do
      :pickup_two ->
        %{game | 
          pending_pickups: game.pending_pickups + 2,
  ...
  defp handle_jack_effect(game, card) do
    cond do
      Card.black_jack?(card) ->
        %{game | 
          pending_pickups: game.pending_pickups + 5,
      ...
      Card.red_jack?(card) and game.pending_pickup_type == :black_jacks ->
        ...
        %{game | 
          pending_pickups: new_pickups,
          pending_pickup_type: pickup_type
        }
