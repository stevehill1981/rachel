defmodule Rachel.Games.GameLiveViewIntegrationTest do
      ...
      if Game.has_valid_play?(game, hd(game.players)) do
        ...
        case valid_plays do
          [{_card, index} | _] ->
            ...
            # Turn advanced
            assert progressed_game.current_player_index != 0
  ...
  describe "user interface edge cases" do
    test "handles rapid card selection and deselection" do
      ...
      rapid_selections = [
        ...
        {:deselect_card, 2},
        {:select_card, 0},
        # Double-select same card
        {:select_card, 0},
        {:deselect_card, 0},
        # Double-deselect same card
        {:deselect_card, 0}
      ]
    ...
    test "handles invalid card combinations in UI" do
      ...
      game = %{game | players: [alice, bob], current_card: %Card{suit: :hearts, rank: 6}}
      ...
      invalid_selection = [
        # hearts 5 (valid)
        {:select_card, 0},
        # spades 7 (invalid with hearts 5)
        {:select_card, 1},
        {:play_selected, []}
      ]
  ...
  describe "ai interaction edge cases" do
    test "handles AI taking too long to play" do
      ...
      # Simulate AI timeout
      timeout_assigns = %{socket_assigns | ai_thinking: false, ai_timeout: 0}
  ...
  describe "game lifecycle edge cases" do
    test "handles game ending during multi-card selection" do
      ...
      socket_assigns = %{
        game: game,
        player_id: "alice",
        # Card selected but not played yet
        selected_cards: [0]
      }
      ...
      updated_assigns = %{
        socket_assigns
        | game: finished_game,
          # Should clear selections when game ends
          selected_cards: []
      }
          ...
          if assigns.selected_cards != [] and
            ...
            case Game.play_card(assigns.game, assigns.player_id, assigns.selected_cards) do
              ...
              {:error, _} ->
                ...
                %{assigns | selected_cards: []}
            end
          else
            # Clear selections if can't play
            %{assigns | selected_cards: []}
          end
      end
    end)
