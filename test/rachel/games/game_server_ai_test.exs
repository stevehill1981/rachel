defmodule Rachel.Games.GameServerAITest do
  ...
  describe "AI turn scheduling" do
    test "schedules AI turn when AI player is current", %{game_id: _game_id} do
      ...
      {game_id, pid} =
        GameServerBuilder.start_server()
    ...
    test "schedules AI turn after human plays", %{game_id: _game_id} do
      ...
      if valid_card do
        ...
        game_with_no_valid_plays =
          GameBuilder.set_current_card(
            initial_game,
            # Something human can't match
            GameBuilder.card({:diamonds, :king})
          )
    ...
    test "AI draws card when no valid plays", %{game_id: _game_id} do
      ...
      game =
        GameBuilder.two_player_game("ai-1", "human-1")
        |> GameBuilder.set_current_player("ai-1")
        # AI can't play on this
        |> GameBuilder.set_current_card(GameBuilder.card({:hearts, 6}))
        |> GameBuilder.give_cards("ai-1", [
          # Can't play - wrong suit/rank
          GameBuilder.card({:clubs, 7}),
          # Can't play - wrong suit/rank
          GameBuilder.card({:diamonds, :king})
        ])
        |> GameBuilder.give_cards("human-1", [
          # Human has a valid card
          GameBuilder.card({:hearts, 8})
        ])
