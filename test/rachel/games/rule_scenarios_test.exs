defmodule Rachel.Games.RuleScenariosTest do
  ...
  describe "deck exhaustion and reshuffling" do
    test "deck reshuffles when exhausted during card draw" do
      ...
      discard_pile = [
        ...
        game.current_card  # Keep current card on top
      ]
      ...
      [p1, p2] = game.players
      p1 = %{p1 | hand: [%Card{suit: :hearts, rank: 8}]}  # No valid play for Black Jack
      game = %{game | players: [p1, p2]}
      ...
      updated_p1 = hd(new_game.players)
      assert length(updated_p1.hand) == 6  # Original 1 + 5 drawn
  ...
  describe "game continuation until one player remains" do
    test "game continues after first player wins until only one remains" do
      ...
      p1 = %{p1 | hand: [%Card{suit: :hearts, rank: 7}]}  # Can play and win
      ...
      active_players = Enum.reject(new_game.players, fn player -> player.id in new_game.winners end)
      ...
      active_after_two = Enum.reject(game_with_two_winners.players, fn player -> player.id in game_with_two_winners.winners end)
      assert length(active_after_two) == 2  # p2 and p4 remain
      
      game_with_three_winners = %{new_game | winners: ["p1", "p3", "p2"]}
      active_after_three = Enum.reject(game_with_three_winners.players, fn player -> player.id in game_with_three_winners.winners end)
      assert length(active_after_three) == 1  # Only p4 remains (the loser)
    end
    ...
    test "turn order preserved after player elimination" do
      ...
      [p1, p2, p3, p4] = game.players
      p1 = %{p1 | hand: [%Card{suit: :hearts, rank: 6}]}  # p1 can win (no special effect)
      ...
      p2 = %{p2 | hand: [%Card{suit: :hearts, rank: 9}]}  # p2 can win (matches suit)
      ...
      active_players = Enum.reject(game_after_p2_wins.players, fn player -> 
  ...
  describe "starting card has no effect" do
    test "starting special cards don't trigger effects" do
      ...
      [p1, p2] = game.players
      p1 = %{p1 | hand: [%Card{suit: :hearts, rank: 5}]}  # Can play on 2H by suit
