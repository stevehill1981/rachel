defmodule Rachel.Games.CriticalBugFixesTest do
  ...
  describe "comprehensive card conservation" do
    test "card count remains exactly 52 through all operations" do
      ...
      exploit_attempts = [
        # Duplicate indices
        fn g -> Game.play_card(g, "player1", [0, 0, 0]) end,
        # Empty array
        fn g -> Game.play_card(g, "player1", []) end,
        # Negative index
        fn g -> Game.play_card(g, "player1", [-1]) end,
        # Out of bounds
        fn g -> Game.play_card(g, "player1", [999]) end,
        # Valid draw
        fn g -> Game.draw_card(g, "player1") end
      ]

      final_game =
        Enum.reduce(exploit_attempts, game, fn attempt, acc ->
          case attempt.(acc) do
