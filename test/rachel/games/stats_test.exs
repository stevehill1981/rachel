defmodule Rachel.Games.StatsTest do
  ...
  describe "record_card_played/3" do
    ...
    test "records multiple special cards", %{stats: stats} do
      cards = [
        Card.new(:hearts, 2),      # Pickup 2
        Card.new(:spades, 7),      # Skip
        Card.new(:hearts, :queen), # Reverse
        Card.new(:spades, :jack),  # Black jack
        Card.new(:hearts, :ace)    # Choose suit
      ]
      new_stats = Stats.record_card_played(stats, "p1", cards)
  ...
  describe "format_stats/1" do
    test "formats complete game stats" do
      ...
      assert formatted.game.winner == "p1"  # First winner
      ...
      assert p1_stats.won == true  # First place
      
      assert p2_stats.cards_played == 0
      assert p2_stats.cards_drawn == 2
      assert p2_stats.won == false  # Second place doesn't count as won
    end
    ...
    test "handles incomplete game" do
      ...
      assert length(formatted.players) == 2  # Still shows players with scores
    end
  end
  ...
  describe "special card detection" do
    test "identifies all special cards correctly" do
      ...
      special_cards = [
        Card.new(:hearts, 2),      # Pickup 2
        Card.new(:spades, 7),      # Skip
        Card.new(:hearts, :queen), # Reverse (queens reverse, not 8s)
        Card.new(:spades, :jack),  # Black jack
        Card.new(:hearts, :ace)    # Choose suit
      ]
    ...
    test "normal cards don't count as special" do
      ...
      normal_cards = [
        Card.new(:hearts, 3),
        Card.new(:spades, 8),  # 8s are NOT special
