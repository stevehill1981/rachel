defmodule Rachel.Games.RulesVerificationTest do
  ...
  test "comprehensive rules verification - all major rules work together" do
    ...
    # Valid play available
    p1 = %{p1 | hand: [%Card{suit: :hearts, rank: 6} | tl(p1.hand)]}
    ...
    # Play 1 two
    assert {:ok, game_one_two} = Game.play_card(game2, "p2", [0])
    assert game_one_two.pending_pickups == 4

    # Or play both (different strategic choice)  
    # Play both twos
    assert {:ok, game_both_twos} = Game.play_card(game2, "p2", [0, 1])
  ...
  test "deck reshuffling works correctly" do
    ...
    [p1, p2] = game.players
    # Can't play on Black Jack
    p1 = %{p1 | hand: [%Card{suit: :spades, rank: 8}]}
    ...
    total_cards_after =
      length(final_game.deck.cards) + length(updated_p1.hand) + length(final_game.discard_pile)

    # At least the cards we started with
    assert total_cards_after >= 7
  ...
  test "game ends only when one player remains" do
    ...
    active_after_2 =
      Enum.reject(game_2_winners.players, fn p -> p.id in game_2_winners.winners end)
    ...
    active_after_3 =
      Enum.reject(game_3_winners.players, fn p -> p.id in game_3_winners.winners end)
  ...
  test "starting cards have no effect while played cards do" do
    ...
    [p1, p2] = game.players
    # Add playable card
    p1 = %{p1 | hand: [%Card{suit: :hearts, rank: 5} | p1.hand]}
    # Add card for p2
    p2 = %{p2 | hand: [%Card{suit: :clubs, rank: 6} | p2.hand]}
    game = %{game | players: [p1, p2]}
    ...
    # Replace first card with 2H
    p2 = %{p2 | hand: [%Card{suit: :hearts, rank: 2} | tl(p2.hand)]}
    game2 = %{game2 | players: [p1_after, p2]}

    assert {:ok, after_played_two} = Game.play_card(game2, "p2", [0])
    # Now it has effect
    assert after_played_two.pending_pickups == 2
  end
