defmodule Rachel.Games.GameRealWorldIntegrationTest do
      ...
      if length(alice.hand) > 0 and Game.has_valid_play?(game, alice) do
        ...
        case valid_plays do
          [{_card, index} | _] ->
            ...
            case result do
              # Expected - turn already advanced
              {:error, :not_your_turn} -> :ok
              # Expected - card already played
              {:error, :invalid_card_index} -> :ok
              # Might be valid if she has more cards
              {:ok, _} -> :ok
  ...
  describe "family game scenarios" do
    test "handles kids making invalid moves repeatedly" do
      ...
      game = %{game | current_card: %Card{suit: :hearts, rank: :ace}, nominated_suit: :hearts}
      ...
      invalid_attempts = [
        # Wrong suit
        Game.play_card(game, "child", [0]),
        # Wrong suit
        Game.play_card(game, "child", [1]),
        # Wrong suit
        Game.play_card(game, "child", [2]),
        # Multiple wrong suits
        Game.play_card(game, "child", [0, 1])
      ]
  ...
  describe "competitive gaming scenarios" do
    test "handles sore loser rage quitting" do
      ...
      # Last card
      winner = %{winner | hand: [%Card{suit: :hearts, rank: 5}]}
      # Many cards
      loser = %{loser | hand: Enum.take(generate_cards(), 15)}
  ...
  describe "long game scenarios" do
    ...
    test "handles memory pressure from large hands" do
      ...
      game = %{game | players: [alice, bob], deck: small_deck, current_player_index: 0}
