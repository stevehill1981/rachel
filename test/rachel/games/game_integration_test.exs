defmodule Rachel.Games.GameIntegrationTest do
  ...
  describe "full game simulations" do
    test "complete 2-player game from start to finish" do
      ...
      assert initial_total_cards == 52,
             "Expected 52 cards, got #{initial_total_cards}. Breakdown: hands=#{Enum.sum(Enum.map(game.players, fn p -> length(p.hand) end))}, deck=#{length(game.deck.cards)}, discard=#{length(game.discard_pile)}, current=1"
      ...
      # Sanity check - games shouldn't take forever
      assert turns_played < 200
      ...
      assert final_total_cards == 52,
             "Cards were lost during gameplay! Started: #{initial_total_cards}, Ended: #{final_total_cards}"
    end
  ...
  describe "edge case scenarios" do
    ...
    test "complex stacking chains with multiple 2s" do
      ...
      # Replace first with 2
      p1 = %{p1 | hand: [%Card{suit: :spades, rank: 2} | tl(p1.hand)]}
      # Replace first with 2
      p2 = %{p2 | hand: [%Card{suit: :diamonds, rank: 2} | tl(p2.hand)]}
      # Replace first with non-2
      p3 = %{p3 | hand: [%Card{suit: :clubs, rank: 8} | tl(p3.hand)]}
    ...
    test "black jack stacking and red jack countering" do
      ...
      [p1, p2, p3] = game.players
      # Replace first with black jack
      p1 = %{p1 | hand: [%Card{suit: :clubs, rank: :jack} | tl(p1.hand)]}
      # Replace first with red jack
      p2 = %{p2 | hand: [%Card{suit: :hearts, rank: :jack} | tl(p2.hand)]}
      # Replace first with 5
      p3 = %{p3 | hand: [%Card{suit: :diamonds, rank: 5} | tl(p3.hand)]}
      game = %{game | players: [p1, p2, p3], current_player_index: 0}

      # Player 1 plays black jack (stacking to 10 total pickup)
      {:ok, game} = Game.play_card(game, "player1", [0])
      # 5 + 5 = 10
      assert game.pending_pickups == 10
    ...
    test "game with direction reversals and skips" do
      ...
      # Replace first with queen
      p1 = %{p1 | hand: [%Card{suit: :hearts, rank: :queen} | tl(p1.hand)]}
      # Replace first with 7
      p2 = %{p2 | hand: [%Card{suit: :spades, rank: 7} | tl(p2.hand)]}
      # Replace first with queen  
      p3 = %{p3 | hand: [%Card{suit: :hearts, rank: :queen} | tl(p3.hand)]}
      # Replace first with 3
      p4 = %{p4 | hand: [%Card{suit: :hearts, rank: 3} | tl(p4.hand)]}
      ...
      # Any valid next player
      assert game.current_player_index in [0, 1, 3]
