defmodule Rachel.Games.GameTest do
  ...
  describe "play_card/3" do
    ...
    test "cards are properly managed in deck and discard pile", %{game: game} do
      ...
      total_in_deck = Rachel.Games.Deck.size(game.deck)
      ...
      if total_in_hands > 0 do
        ...
        if length(first_player.hand) > 0 do
          case Game.play_card(game, first_player.id, [0]) do
            {:ok, new_game} ->
              new_total_in_hands = Enum.sum(Enum.map(new_game.players, fn p -> length(p.hand) end))
              new_total_in_deck = Rachel.Games.Deck.size(new_game.deck)
              new_total_in_discard = length(new_game.discard_pile)
              new_current_card_count = if new_game.current_card, do: 1, else: 0
              
              new_total_cards = new_total_in_hands + new_total_in_deck + new_total_in_discard + new_current_card_count
    ...
    test "game ends when only one player remains" do
      ...
      game = %{game | 
        ...
        current_player_index: 2,  # Start with p3's turn
        winners: ["p1", "p2"]     # p1 and p2 already won
      }
      
      [p1, p2, p3] = game.players
      p1 = %{p1 | hand: []}  # Already won
      p2 = %{p2 | hand: []}  # Already won
      p3 = %{p3 | hand: [%Card{suit: :hearts, rank: :queen}]}  # Last remaining player - will be loser
      ...
      assert game.status == :finished  # Game should end immediately - only p3 remained active
