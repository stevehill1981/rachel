defmodule Rachel.AI.EnhancedAIPlayerTest do
  ...
  describe "choose_play/2" do
    ...
    test "chooses to draw card when no valid plays available", %{
      game: game,
    ...
    test "chooses to play valid card when available", %{
      game: game,
      ai_player_data: ai_player_data,
      player_struct: player_struct
    } do
      ...
      # Indices of valid cards
      assert hd(cards) in [0, 2]
    ...
    test "considers special cards based on personality", %{game: game} do
      ...
      # Should choose special cards more than half the time
      assert special_card_choices > 5

      :meck.unload(Process)
    end

    test "handles game with pending pickups", %{
      game: game,
      ai_player_data: ai_player_data,
      player_struct: player_struct
    } do
      ...
      hand = [
        ...
        # Can't play with pending pickups
        %Card{suit: :hearts, rank: 9},
        # Can't play
      ...
      if action == :play_cards do
        ...
        # Might play either card due to randomness
        assert hd(cards) in [0, 1]
      end

      :meck.unload(Process)
    end

    test "handles game with ace played and suit nomination", %{
      game: game,
      ai_player_data: ai_player_data,
      player_struct: player_struct
    } do
      ...
      assert action == :play_cards
      # Should play the spades card at index 1
      assert cards == [1]
    ...
    test "respects thinking time based on personality", %{game: game} do
      ...
      # At least 100ms
      assert thinking_time > 100

      :ets.delete(sleep_times)
      :meck.unload(Process)
    end

    test "updates AI memory during play", %{
      game: game,
      ai_player_data: ai_player_data,
      player_struct: player_struct
    } do
    ...
    test "handles multiple valid plays with different scores", %{
      ...
      # Should not always pick the same card
      assert length(unique_choices) > 1
