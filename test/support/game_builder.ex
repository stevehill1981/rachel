defmodule Test.GameBuilder do
  ...
  alias Rachel.Games.{Game, Card, Deck}
  ...
  def set_nominated_suit(game, suit) when suit in [:hearts, :diamonds, :clubs, :spades, :pending, nil] do
  ...
  def special_card_scenario(card, target_player_id) do
    ...
    |> set_current_card(%Card{suit: card.suit, rank: 3}) # Safe card to play on
  ...
  def winning_scenario(player_id, winning_card) do
    ...
    |> set_current_card(%Card{suit: winning_card.suit, rank: 5}) # Safe to play on
    |> give_cards(player_id, [winning_card]) # Only one card left
  end
