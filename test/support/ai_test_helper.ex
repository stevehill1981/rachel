defmodule Test.AITestHelper do
  ...
  def ai_scenario(:prefer_non_ace, ai_id) do
    ...
    ai_turn_game([
      GameBuilder.card({:hearts, 3}),    # Matches suit - should choose this
      GameBuilder.card({:spades, :ace})  # Ace - should avoid
    ], GameBuilder.card({:hearts, :king}))
    |> GameBuilder.set_current_player(ai_id)
  end
  ...
  def ai_scenario(:stack_twos, ai_id) do
    ...
    |> GameBuilder.give_cards(ai_id, [
      GameBuilder.card({:spades, 2}),   # Can stack
      GameBuilder.card({:hearts, 3})    # Can't play
    ])
  end
  ...
  def ai_scenario(:nominate_suit, ai_id) do
    ...
    |> GameBuilder.give_cards(ai_id, [
      GameBuilder.card({:spades, 2}),
      GameBuilder.card({:spades, 3}),    # Most common suit
  ...
  def ai_scenario(:counter_black_jack, ai_id) do
    ...
    |> GameBuilder.give_cards(ai_id, [
      GameBuilder.card({:hearts, :jack}),  # Red jack cancels
      GameBuilder.card({:clubs, 2})        # Can't play
    ])
  end
  ...
  def ai_scenario(:skip_opponent, ai_id) do
    ...
    ai_turn_game([
      GameBuilder.card({:hearts, 7}),    # Skip card
      GameBuilder.card({:hearts, 10})    # Alternative
