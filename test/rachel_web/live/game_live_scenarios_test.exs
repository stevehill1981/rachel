defmodule RachelWeb.GameLiveScenariosTest do
  ...
  describe "helper function coverage" do
    ...
    test "handles get_player_name_by_id with various inputs" do
      ...
      test_messages = [
        ...
        {:suit_nominated, %{player_id: "nominator", suit: :hearts, game: %{players: [%{id: "nominator", name: "Nominator", hand: [], is_ai: false}], status: :playing, current_player_index: 0, direction: :clockwise, winners: [], deck: %Deck{cards: [], discarded: []}, current_card: %Card{suit: :hearts, rank: :ace}, discard_pile: [], pending_pickups: 0, pending_skips: 0}}}
      ]
  ...
  describe "PubSub message edge cases" do
    test "handles malformed PubSub messages" do
      ...
      edge_case_messages = [
        {:game_started, %{id: "test", status: :playing, players: [], deck: %Deck{cards: [], discarded: []}, current_card: %Card{suit: :hearts, rank: :ace}, current_player_index: 0, direction: :clockwise, discard_pile: [], pending_pickups: 0, pending_skips: 0, winners: []}},
        {:player_won, %{player_id: "winner", game: %{winners: ["winner"], players: [], status: :finished, deck: %Deck{cards: [], discarded: []}, current_card: %Card{suit: :hearts, rank: :ace}, current_player_index: 0, direction: :clockwise, discard_pile: [], pending_pickups: 0, pending_skips: 0}}},
        {:card_drawn, %{player_id: "drawer", game: %{players: [], status: :playing, deck: %Deck{cards: [], discarded: []}, current_card: %Card{suit: :hearts, rank: :ace}, current_player_index: 0, direction: :clockwise, discard_pile: [], pending_pickups: 0, pending_skips: 0, winners: []}}}
      ]
    ...
    test "handles player messages with missing player data" do
      ...
      incomplete_messages = [
        ...
        {:suit_nominated, %{player_id: "test", suit: :hearts, game: %{players: [%{id: "test", name: "Test", hand: [], is_ai: false}], status: :playing, current_player_index: 0, direction: :clockwise, winners: [], deck: %Deck{cards: [], discarded: []}, current_card: %Card{suit: :hearts, rank: :ace}, discard_pile: [], pending_pickups: 0, pending_skips: 0}}}
      ]
  ...
  describe "complex game scenarios" do
    ...
    test "handles game with extreme card counts" do
      ...
      if view_state.socket.assigns.game do
        ...
        updated_players = Enum.with_index(game.players, fn player, index ->
          case index do
            0 -> %{player | hand: Enum.take(game.deck.cards, 15)} # Many cards
            1 -> %{player | hand: [hd(game.deck.cards)]} # One card
            _ -> %{player | hand: []} # No cards
          end
        end)
